...
static void do_block_unrollavx128(int lda, int M, int N, int K, double* A, double* B, double* C)
{
    ...
    for (int k=0; k<K; k+=2) {
        for (int j=0; j<N; j+=2) {
            ...
            b1 = _mm_load1_pd(B+k+1+j*lda);
            b2 = _mm_load1_pd(B+k+(j+1)*lda);
            b3 = _mm_load1_pd(B+k+1+(j+1)*lda);

            for (int i=0; i<M; i+=2) {
                a0 = _mm_load_pd(A+i+k*lda);
                a1 = _mm_load_pd(A+i+(k+1)*lda);

                c0 = _mm_load_pd(C+i+j*lda);
                c1 = _mm_load_pd(C+i+(j+1)*lda);
                ...
                _mm_store_pd(C+i+j*lda,c0);
                _mm_store_pd(C+i+(j+1)*lda,c1);
            }
        }
    }
}
